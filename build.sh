...
if [[ (! -z "$1") &&  ("$1" == "force") ]]; then
	...
	docker build -f ./dockerfile/dockerfile_sparkbase -t spark-base:3.0.3 -t spark-base:latest .
	docker build -f ./dockerfile/dockerfile_sparkmaster -t spark-master:3.0.3 -t spark-master:latest .
	docker build -f ./dockerfile/dockerfile_sparkthrift -t spark-thrift:3.0.3 -t spark-thrift:latest .
	docker build -f ./dockerfile/dockerfile_sparkworker -t spark-worker:3.0.3 -t spark-worker:latest .
